<script lang="ts">
import { ref } from 'vue'
import {  getProductsFromSeason } from '../services/film'
...
export default {
  async created() {
    ...
    if (season) {
      this.products = await getProductsFromSeason(season.toString())
...
<template>
  ...
  <h1 class="header">Các loại nông sản thu hoạch vào {{season }}  </h1>
